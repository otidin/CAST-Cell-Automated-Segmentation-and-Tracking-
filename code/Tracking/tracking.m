function [spots dist] = tracking(spots, doLinksOnly)

 %tracking(spots, track_opts, opts)
...
%   end
  ...
  for i=1:nframes
    ...
    if (prev_npts > 0 & npts > 0)
      ...
      ends(ends==0) = Inf;
      trans_dist = (mutual_dist.' < Inf) * min(mutual_dist(mutual_dist>0));
...
if( ~doLinksOnly )
  ...
  % try to tweak the weights

%good old weights
%   split_weight = 0.8*split_weight;
...
%   alt_weight = 1*alt_weight;
  ...
  disp(['mutual dist: ' num2str(mean(mutual_dist(isfinite(mutual_dist)) ))]);
  disp(['merge_weight: ' num2str(mean(merge_weight(isfinite(merge_weight)) ))]);
  disp(['split_weight: ' num2str(mean(split_weight(isfinite(split_weight)) ))]);
  disp(['alt_dist: ' num2str(mean(alt_dist(isfinite(alt_dist)) ))]);
  ...
  trans_dist = (dist(1:nends+ninterm, 1:nstarts+ninterm).' < Inf) * 0.01; %try something different
  trans_dist(trans_dist == 0) = Inf;

...
function guiTraces_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    global axesData axesGif axesGif2;
    axesData = handles.axes1;
    axesGif = handles.axes2;
    axesGif2 = handles.axes3;
...
colormap jet
 ...
 function  updatePlotGif2()
    
    global gif2 t axesGif2 gmap2 longTraces k caxisMin caxisMax;  
        
    axes(axesGif2);
    
        
    if(exist(['snapShots/' num2str(longTraces(k)) '_' num2str(t) '.png'],'file'))
        im2 = imread(['snapShots/' num2str(longTraces(k)) '_' num2str(t) '_2.png']);
        imagesc(im2);
        if( caxisMax > 0 )
            caxis([caxisMin,caxisMax])
        else
            p=0.001;
            caxis([quantile(im2(:),p),quantile(im2(:),1-p)])
        end
        set(axesGif2,'visible','off')
    end
...
function KeyPress(Source, EventData)
    ...
    handles = guidata(gcf);
    global t speed gmap gmap2 axesData moviePath k tracks signal traj ind longTraces peakMatrix divMatrix Nframes divisions Ntraces gif gif2 expe goodTraces;
